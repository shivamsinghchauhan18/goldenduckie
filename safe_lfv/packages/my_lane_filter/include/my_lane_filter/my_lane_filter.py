...
class LaneFilterHistogram(Configurable, LaneFilterInterface):
    ...
    def __init__(self, configuration):
        ...
        self.cov_mask = [self.sigma_d_mask, self.sigma_phi_mask] # [1.0, 2.0]

        self.prev_time = time.time()
        # self.inlier_alpha = 4.0
        self.inlier_alpha = 1.0
        self.neighbor_dist = 0.25
        # self.neighbor_dist = 0.05
    ...
    def predict(self, dt, v, w):
        ...
        time_correction = (time.time() - self.prev_time) * 30.0 # default cov_mask works well for 30 fps
        self.prev_time = time.time()
        for k in range(self.curvature_res):
            ...
            cov_mask = [(x * time_correction) for x in self.cov_mask]
            gaussian_filter(p_belief, cov_mask,
    ...
    def get_inlier_segments(self, segments, d_max, phi_max, sim_mode=False):
        inlier_segments = []
        for segment in segments:
            d_s, phi_s, l, w = self.generateVote(segment)
            if abs(d_s - d_max) < (self.inlier_alpha * self.delta_d) and abs(phi_s - phi_max)< (self.inlier_alpha * self.delta_phi):
    ...
    def loginfo(self, s):
		rospy.loginfo('[lane_filter.py] %s' % s)