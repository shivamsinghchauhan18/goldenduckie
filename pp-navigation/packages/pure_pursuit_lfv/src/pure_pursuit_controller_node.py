...
class pure_pursuit(object):
    def __init__(self):
        ...
        if self.node_namespace == '/default/':
            ...
            self.lookahead_distance = 0.25
            params = Parameters(v_min=0.2, v_max=0.5, dv_neg=-0.25, dv_pos=0.03, w_gain_min=0.62, w_gain_max=2.6, num_y_vup=1, num_r_vdown=3, right_wg_scale=1.05, func='x2', obs_stop_thresh=0.3, lane_width=0.36, robot_width=0.12, white_dir_correction=0.5, yellow_dir_correction=0.5)
        ...
        self.red_line_distance = self.setupParam('~red_line_distance', 0.2)  # Distance threshold for red line detection
        self.turn_omega = self.setupParam('~turn_omega', -2.0)  # Angular velocity for 90-degree turn (radians/s, negative for right turn)
        self.turn_duration = self.setupParam('~turn_duration', 1.0)  # Duration for 90-degree turn (seconds)
        ...
        self.detection_list = []
        self.red_line_visible = False  # Track red line visibility
        self.red_line_last_seen = None  # Timestamp of last red line detection
        self.turn_in_progress = False  # Flag for 90-degree turn
        self.turn_start_time = None  # Timestamp when turn starts
    ...
    def updateFilteredLineSeg(self, gp_segment_list):
        ...
        # Get points from SegmentList message and convert them to numpy
        white_points_p0, white_points_p1, yellow_points_p0, yellow_points_p1, red_points_p0, red_points_p1 = self.segmentUnpack(gp_segment_list)

        # Process red lines
        num_red = 0
        if len(red_points_p0) > 0:
            red_points, _, num_red, _, _ = self.preprocessPoints(red_points_p0, red_points_p1)
            if num_red > 0 and np.min(red_points[:, 0]) < self.red_line_distance:
                self.red_line_visible = True
                self.red_line_last_seen = rospy.get_time()
                self.loginfo("Red line detected, continuing to follow lane")
            else:
                self.red_line_visible = False
        else:
            self.red_line_visible = False

        # Check if red line just went out of sight and trigger 90-degree turn
        if not self.red_line_visible and self.red_line_last_seen is not None:
            time_since_last_seen = rospy.get_time() - self.red_line_last_seen
            if time_since_last_seen < 0.5 and not self.turn_in_progress:  # Within 0.5s of losing red line
                self.turn_in_progress = True
                self.turn_start_time = rospy.get_time()
                self.loginfo("Red line out of sight, starting 90-degree turn")

        # Perform 90-degree turn if in progress
        if self.turn_in_progress:
            ...
            if elapsed_time < self.turn_duration:
                ...
                car_cmd_msg.v = 0.0  # Stop linear motion
                car_cmd_msg.omega = self.turn_omega  # Rotate on spot
                self.publishCmd(car_cmd_msg)
                self.loginfo("Executing 90-degree turn")
                return
            else:
                self.turn_in_progress = False
                self.red_line_last_seen = None
                self.loginfo("Completed 90-degree turn")

        # Continue with lane following if no turn is in progress
    ...
    def segmentUnpack(self, gp_segment_list):
        ...
        if gp_segment_list is not None:
            for segment in gp_segment_list.segments:
                ...
                if color == Segment.WHITE:
                    ...
                    red_points_p0.append(p0)
                    red_points_p1.append(p1)
        ...
        red_points_p0 = np.array(red_points_p0, ndmin=2)
        red_points_p1 = np.array(red_points_p1, ndmin=2)
        return white_points_p0, white_points_p1, yellow_points_p0, yellow_points_p1, red_points_p0, red_points_p1
